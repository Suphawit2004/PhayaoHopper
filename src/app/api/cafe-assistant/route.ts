...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ mode, fallbackReason, provider: mode === "ai" ? "gemini" : null,
    ...
    cafes: matched.map(c => ({ slug: c.slug, name: c.name[lang] ?? c.name.th, openTime: c.openTime, closeTime: c.closeTime, closedDays: c.closedDays, address: c.address[lang] ?? c.address.th }))
  });
}
