...
describe("community photo authentication and profile ownership", () => {
  ...
  it("filters by the authenticated ID even for an admin account", async () => {
    const query = { is: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockResolvedValue({ data: [], error: null }) };
    ...
    expect(query.is).toHaveBeenCalledWith("review_batch", null);
  });
  it("includes an older linked photo scoped to the requested cafe", async () => {
    ...
    const query = { is: vi.fn().mockReturnThis(), select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockResolvedValue({ data: [], error: null }), maybeSingle: vi.fn().mockResolvedValue({ data: row, error: null }) };
  ...
  it("does not sign a linked photo hidden by row permissions or from another cafe", async () => {
    const query = { is: vi.fn().mockReturnThis(), select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), limit: vi.fn().mockResolvedValue({ data: [], error: null }), maybeSingle: vi.fn().mockResolvedValue({ data: null, error: null }) };
    const storage = { from: vi.fn() };
