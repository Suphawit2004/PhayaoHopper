...
export default function OwnerAssignment({slug,currentId}:{slug:string;currentId:string}){
 ...
 return <section className="feature-card"><h2>{lang==="th"?"กำหนดเจ้าของร้าน":"Assign cafe owner"}</h2><p>{lang==="th"?"ให้เจ้าของร้านส่งรหัสบัญชีจากหน้าสำหรับเจ้าของร้าน แล้วตรวจชื่อบัญชีก่อนบันทึกสิทธิ์":"Ask the cafe owner to share their account ID from the owner page, then verify the account before granting access."}</p>
 <label className="grid gap-2 mt-3">{lang==="th"?"รหัสบัญชี":"Account ID"}<input className="border rounded-lg p-3" value={id} maxLength={36} onChange={e=>{setId(e.target.value.trim());setVerified(null);setError("");}}/></label>
