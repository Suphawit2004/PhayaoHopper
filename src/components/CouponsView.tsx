...
export default function CouponsView({ coupons, cafes, serverNow, error }: { coupons: Coupon[]; cafes: { slug: string; name: { th: string; en: string } }[]; serverNow: number; error: boolean }) {
  ...
  return <div className="feature-page"><p className="text-sm font-semibold text-coffee">PHAYAOHOPPER · DEMO REWARDS</p><h1>{th ? "คูปองของฉัน" : "My coupons"}</h1>
    ...
    <p role="status" className="my-4">{notice}</p>
    {error ? <div role="alert"><p>{th ? "โหลดคูปองไม่สำเร็จ" : "Could not load coupons."}</p><button className="ui-secondary" onClick={() => router.refresh()}>{th ? "ลองใหม่" : "Retry"}</button></div> : !coupons.length ? <div className="py-12 border-y border-[#d8c9b7]"><h2>{th ? "ยังไม่มีคูปอง" : "No coupons yet"}</h2><p>{th ? "รีวิวรับ 5 บาท หรือแนบรูปสาธารณะตั้งแต่ 3 รูปเพื่อรับ 10% แทน" : "Review for ฿5, or attach at least 3 public photos for 10% instead."}</p><Link href="/visited" className="ui-secondary mt-4">{th ? "เลือกร้านที่เคยไป →" : "Choose a visited cafe →"}</Link></div> :
      <ul className="grid gap-6 md:grid-cols-2">{coupons.map(c => {
        ...
        return <li key={c.id} className="border-y border-[#d8c9b7] py-6"><div className="flex justify-between gap-3"><h2 className="text-xl">{cafe ? <Link className="underline underline-offset-4" href={`/cafes/${c.cafe_slug}`}>{cafe.name[lang]}</Link> : c.cafe_slug}</h2><span className={state === "available" ? "text-emerald-800" : "text-espresso/60"}>{label}</span></div>
          <p className="my-5 text-4xl font-bold text-coffee">{c.reward === "10_percent" ? "10%" : "5 ฿"}<span className="ml-3 text-sm font-normal">{th ? "ส่วนลดทดลอง" : "Demo discount"}</span></p>
