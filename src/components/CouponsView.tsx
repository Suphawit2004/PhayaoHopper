...
export default function CouponsView({ coupons, cafes, serverNow, error }: { coupons: Coupon[]; cafes: { slug: string; name: { th: string; en: string } }[]; serverNow: number; error: boolean }) {
  ...
  return <div className="feature-page"><p className="text-sm font-semibold text-coffee">PHAYAOHOPPER · DEMO REWARDS</p><h1>{th ? "คูปองของฉัน" : "My coupons"}</h1>
    ...
    {error ? <div role="alert"><p>{th ? "โหลดคูปองไม่สำเร็จ" : "Could not load coupons."}</p><button className="ui-secondary" onClick={() => router.refresh()}>{th ? "ลองใหม่" : "Retry"}</button></div> : !coupons.length ? <div className="coupon-empty py-12"><h2>{th ? "ยังไม่มีคูปอง" : "No coupons yet"}</h2><p>{th ? "รีวิวรับ 5 บาท หรือแนบรูปสาธารณะตั้งแต่ 3 รูปเพื่อรับ 10% แทน" : "Review for ฿5, or attach at least 3 public photos for 10% instead."}</p><Link href="/visited" className="ui-secondary mt-4">{th ? "เลือกร้านที่เคยไป →" : "Choose a visited cafe →"}</Link></div> :
