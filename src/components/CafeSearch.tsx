...
export default function CafeSearch({ variant = "default" }: { variant?: "default" | "navbar" }) {
  ...
  return <div className={`cafe-search ${variant === "navbar" ? "cafe-search-navbar" : ""}`} onBlur={e => {if (!e.currentTarget.contains(e.relatedTarget)) setOpen(false);}}>
    <form onSubmit={e => {e.preventDefault(); setOpen(false); router.push(index >= 0 && matches[index] ? `/cafes/${matches[index].slug}` : all);}}>
      ...
      <button type="submit" className="feature-button" aria-label={lang === "th" ? "ค้นหา" : "Search"}>{variant === "navbar" ? <Icon name="search" aria-hidden="true" /> : (lang === "th" ? "ค้นหา" : "Search")}</button>
    </form>
